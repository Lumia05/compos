...
// Routes
app.get('/', (req, res) => {
  res.json({ service: 'supply-chain-backend', status: 'Online', description: 'Supply Chain Module' });
});

app.get('/api/data', (req, res) => {
  // Simulate some delay for performance testing
  setTimeout(() => {
    res.json({ data: 'Sample data from supply-chain-backend', timestamp: new Date() });
  }, Math.random() * 200);
});
